...
public class TransformerConverter extends AbstractConverter {
    ...
    private void createTwoWindingsTransformer() {
        ...
        double w1 = defineRatio(psseTransformer.getWindingRecord1().getWindv(), baskv1, nomV1, psseTransformer.getCw());
        ...
        if (psseTransformer.getWindingRecord1().getAng() != 0) {
            PhaseTapChangerAdder phaseTapChangerAdder = tfo2W.newPhaseTapChanger()
                .setRegulationMode(PhaseTapChanger.RegulationMode.FIXED_TAP)
                .setRegulating(false)
                .setTapPosition(0);
            List<Double> alphas = new ArrayList<>();
            alphas.add(-psseTransformer.getWindingRecord1().getAng()); // TODO : check angle and angle units (supposed
                                                                       // in degrees)
        ...
        if (psseTransformer.getWindingRecord2().getAng() != 0) {
    ...
    private void createThreeWindingsTransformer() {
        ...
        double w1 = defineRatio(psseTransformer.getWindingRecord1().getWindv(), baskv1, nomV1, psseTransformer.getCw());
        double w2 = defineRatio(psseTransformer.getWindingRecord2().getWindv(), baskv2, nomV2, psseTransformer.getCw());
        double w3 = defineRatio(psseTransformer.getWindingRecord3().getWindv(), baskv3, nomV3, psseTransformer.getCw());
        ...
        if (psseTransformer.getK() != 0 && psseTransformer.getWindingRecord3().getAng() != 0) {
